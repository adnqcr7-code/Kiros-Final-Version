...
// In-memory store of staged patches, keyed by resolved file path.
// A patch must be previewed before it can be applied, mirroring the
// approval-gated pattern used elsewhere in this module.
...
function previewPatch(targetPath, newContent) {
  ...
  const diff = buildLineDiff(oldContent, newContent);
  ...
  pendingPatches.set(resolved, patch);
  logToolEvent({ tool: 'diff.view', targetPath, completed: true });
  return patch;
}

function viewPendingPatch(targetPath) {
  const resolved = safeResolve(targetPath);
  const patch = pendingPatches.get(resolved);
  if (!patch) {
    throw new Error(`No pending patch for: ${targetPath}`);
  }
  return patch;
}
...
async function applyFilePatch(targetPath, flags = {}) {
  ...
  const patch = pendingPatches.get(resolved);
  if (!patch) {
    throw new Error(`No pending patch for: ${targetPath}. Call previewPatch first.`);
  }

  const review = reviewAction({ kind: 'patch', targetPath: resolved });
  if (!await requireApproval(review, flags)) {
    return { completed: false, message: 'Patch application cancelled.' };
  }

  fs.writeFileSync(resolved, patch.newContent, 'utf8');
  pendingPatches.delete(resolved);

  const result = { completed: true, path: resolved, relativePath: patch.relativePath, review };
  logToolEvent({ tool: 'files.patch', targetPath, completed: true, risk: review.level });
  return result;
}
