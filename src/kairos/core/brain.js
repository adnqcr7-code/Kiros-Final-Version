...
// Rough $ per 1K tokens (blended input/output), used only to give the
// budget-limit feature something to compare against. These are estimates,
// not billing-accurate figures, since providers here don't return usage data.
const PROVIDER_RATE_PER_1K = {
  ollama: 0,
  codex: 0,
  offline: 0,
  openai: 0.002,
...
function recordSessionCost(providerId, promptText, replyText) {
  const rate = PROVIDER_RATE_PER_1K[providerId] || 0;
  if (rate === 0) return;
  const tokens = estimateTokens(promptText) + estimateTokens(replyText);
  sessionCostUsd += (tokens / 1000) * rate;
}
...
function postJsonStreaming(url, payload, onLine, options = {}) {
  const body = JSON.stringify(payload);
  const client = url.startsWith('https:') ? https : http;
  ...
  return new Promise((resolve) => {
    const request = client.request(url, {
      ...
      if (response.statusCode < 200 || response.statusCode >= 300) {
        let errText = '';
        response.setEncoding('utf8');
        response.on('data', (chunk) => { errText += chunk; });
        response.on('end', () => {
          resolve({ ok: false, statusCode: response.statusCode, text: errText });
        });
        return;
      }
...
async function askOllama(prompt, history = [], onToken = null) {
  ...
  if (typeof onToken === 'function') {
    ...
    const result = await postJsonStreaming(`${baseUrl}/api/chat`, {
      model,
    ...
    if (!combined && parseFailed) return 'Ollama returned invalid JSON.';
    return combined || 'Ollama returned no response.';
  }
...
async function askBrainRaw(prompt, history = [], onToken = null) {
...
async function askBrain(prompt, history = [], onToken = null) {
  const provider = providerStatus();
  const reply = await askBrainRaw(prompt, history, onToken);
  recordSessionCost(provider.id, prompt, reply);
  return reply;
}
